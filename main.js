...
try {
if (slackWebhookUrl) {
const text = TikTok Shop alert • ${payload.event} for ${payload.product?.title || payload.product?.product_id || 'product'} • Price: ${payload.product?.price?.current ?? 'n/a'} ${payload.product?.price?.currency ?? ''} • Stock: ${payload.product?.availability ?? 'n/a'} • URL: ${payload.product?.url};
await gotScraping({
url: slackWebhookUrl,
method: 'POST',
json: { text },
...
if (!txt) continue;
if (phrases.some((p) => txt.includes(p))) b.click();
}
});
await sleep(500);
} catch {}
...
return {
initProps: safe(window.INIT_PROPS),
sigi: safe(window.SIGI_STATE),
next: safe(window.NEXT_DATA),
ld: scrs,
anyJsonScript,
...
let images = Array.from(document.querySelectorAll('img')).map(i => i.src).filter(Boolean);
...
return { title, priceText, images, ratingText, seller };
});
}
...
const proxy = proxyConfiguration ? await Actor.createProxyConfiguration(proxyConfiguration) : null;
const kv = await Actor.openKeyValueStore('STATE');
const queue = await Actor.openRequestQueue();

for (const url of productUrls) {
    await queue.addRequest({ url: normalizeUrl(url), label: 'PRODUCT' });
}
for (const handle of sellerHandles) {
    const url = `https://www.tiktok.com/@${handle}`;
    await queue.addRequest({ url, label: 'SELLER', userData: { handle } });
}
for (const kw of keywords) {
    const url = `https://www.tiktok.com/search?q=${encodeURIComponent(kw)}`;
    await queue.addRequest({ url, label: 'KEYWORD', userData: { kw } });
}
for (const url of categoryUrls) {
    await queue.addRequest({ url, label: 'CATEGORY' });
}

const crawler = new PuppeteerCrawler({
    requestQueue: queue,
    proxyConfiguration: proxy || undefined,
    maxConcurrency,
    useSessionPool: true,
    ...
    requestHandler: async ({ request, page, response }) => {
        const { label } = request;
        if (response && response.status() >= 400) log.warning(`HTTP ${response.status()} for ${request.url}`);
        ...
        if (label === 'PRODUCT') {
            ...
            if (captureScreenshots) {
                try {
                    ...
                    if (buffer) await Actor.setValue(screenshotKey, buffer, { contentType: 'image/png' });
                } catch (e) {
            ...
            for (const u of unique) await queue.addRequest({ url: normalizeUrl(u), label: 'PRODUCT' });
